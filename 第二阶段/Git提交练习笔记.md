...
💡 四、常见问题速查
 ...
 - 报错信息 解决办法 
  - not a git repository  未进入仓库，执行  cd /e/Git/Tasks  
 - pathspec 'hello.md' did not match any files  文件不存在或路径错，检查  E:\Git\Tasks  里的文件 
 - switch 'm' requires a value   git commit -m  后没加提交说明，补全为  git commit -m "备注"  
 - Author identity unknown  未配置身份，执行  git config --global user.name/email  
 - rejected main -> main  先执行  git pull origin main  同步代码，再推送 
 - unfinished merge  执行  git commit -m "合并备注"  完成合并，或  git merge --abort  取消合并 
